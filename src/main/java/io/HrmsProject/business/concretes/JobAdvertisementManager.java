...
public class JobAdvertisementManager implements JobAdvertisementService {
	...
	public Result add(CreateJobAdvertisementRequests createJobAdvertisement) throws Exception {
		JobAdvertisement jobAdvertisement = this.modelMapperService.forRequest().map(createJobAdvertisement,
		...
		// this.typeOfWorkDao.findById(createJobAdvertisement.getTypeOfWork());
	...
	public Result update(UpdateJobAdvertisementRequests updateJobAdvertisement) {
		JobAdvertisement jobAdvertisement = this.modelMapperService.forRequest().map(updateJobAdvertisement,
				JobAdvertisement.class);
		this.jobAdvertisementDao.save(jobAdvertisement);
	...
	public DataResult<List<GetAllJobAdvertisementResponses>> getAll() {
		...
		List<GetAllJobAdvertisementResponses> jobAdvertisementResponses = jobAdvertisements.stream()
				.map(jobAdvertisement -> this.modelMapperService.forResponse().map(jobAdvertisement,
						GetAllJobAdvertisementResponses.class))
	...
	public Result makeActiveOrPassive(boolean isActive, int advertisementId) {
		...
		String statuMessage = isActive ? "advertisement has been activated." : "advertisement has been passived. ";
	...
	public DataResult<List<GetAllJobAdvertisementResponses>> getJobAdvertisementDetailSorted() {
		...
		List<GetAllJobAdvertisementResponses> response = jobAdvertisements.stream()
				.map(jobAdvertisement -> this.modelMapperService.forResponse().map(jobAdvertisement,
						GetAllJobAdvertisementResponses.class))
				.collect(Collectors.toList());
	...
	@Override
	public DataResult<List<GetAllJobAdvertisementResponses>> getByIsActiveAndEmployerId(boolean isActive,
			int employerId) {
		Sort sort = Sort.by(Sort.Direction.DESC, "applicationDate");
		List<JobAdvertisement> jobAdvertisements = jobAdvertisementDao.getByIsActiveAndEmployer_Id(true, employerId,
				sort);
		List<GetAllJobAdvertisementResponses> response = jobAdvertisements.stream()
				.map(jobAdvertisement -> this.modelMapperService.forResponse().map(jobAdvertisement,
						GetAllJobAdvertisementResponses.class))
	...
	public DataResult<List<GetAllJobAdvertisementResponses>> getByCompanyName(String companyName, boolean isActive) {
		Sort sort = Sort.by(Sort.Direction.DESC, "applicationDate");
		List<JobAdvertisement> jobAdvertisements = jobAdvertisementDao
				.findByEmployer_CompanyNameAndIsActive(companyName, sort, isActive);
		List<GetAllJobAdvertisementResponses> response = jobAdvertisements.stream()
				.map(jobAdvertisement -> this.modelMapperService.forResponse().map(jobAdvertisement,
						GetAllJobAdvertisementResponses.class))
				.collect(Collectors.toList());
	...
	public DataResult<GetByIdJobAdvertisementResponse> getById(int id) {
		...
		GetByIdJobAdvertisementResponse response = this.modelMapperService.forResponse().map(jobAdvertisement,
				GetByIdJobAdvertisementResponse.class);
	...
	public DataResult<List<GetAllJobAdvertisementResponses>> getAllByJobAdvertisementName(String advertisementName) {
		Sort sort = Sort.by(Sort.Direction.DESC, "applicationDate");
		List<JobAdvertisement> jobAdvertisements = jobAdvertisementDao
				.findByAdvertisementNameContainsIgnoreCase(advertisementName, sort);
		List<GetAllJobAdvertisementResponses> response = jobAdvertisements.stream()
				.map(jobAdvertisement -> this.modelMapperService.forResponse().map(jobAdvertisement,
						GetAllJobAdvertisementResponses.class))
				.collect(Collectors.toList());
	...
	@Override
	public DataResult<List<GetAllJobAdvertisementResponses>> getBySectorAndCity(int sectorId, int cityId) {
		List<JobAdvertisement> jobAdvertisements = jobAdvertisementDao
				.findBySector_SectorIdAndCity_CityId(sectorId, cityId);
		List<GetAllJobAdvertisementResponses> response = jobAdvertisements.stream()
				.map(jobAdvertisement -> this.modelMapperService.forResponse().map(jobAdvertisement,
						GetAllJobAdvertisementResponses.class))
				.collect(Collectors.toList());
